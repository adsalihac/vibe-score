...
function statLabel(label: string, value: string | number) {
  return (
    <div className="rounded-xl border border-white/10 bg-white/5 p-3 backdrop-blur-sm transition-colors duration-200 hover:border-[#f7931a]/40">
...
Generated by VibeScore: ${shareableScanUrl}`;
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden px-4 pb-10 pt-14 md:px-10 md:pb-14 md:pt-16">
      ...
      <div className="pointer-events-none absolute left-[-10%] top-[-10%] h-72 w-72 rounded-full bg-[#f7931a]/15 blur-[120px]" />
      <div className="pointer-events-none absolute right-[-8%] top-[18%] h-80 w-80 rounded-full bg-[#ffd600]/10 blur-[140px]" />
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 md:gap-8">
        ...
        >
          <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#f7931a]/60 to-transparent" />
          ...
          <h1 className="mt-5 max-w-4xl text-4xl font-bold leading-tight text-white sm:text-5xl md:text-7xl">
            We investigate code.
          </h1>
          ...
          <div className="mt-6 grid gap-3 lg:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
              ...
              <select
                className="mt-2 w-full rounded-xl border border-white/10 bg-black/45 px-4 py-3 text-sm text-white outline-none transition focus:border-[#f7931a]"
                value={rulePack}
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-xs text-white">
                {activeRulePack.emphasis.map((item) => (
                  <span key={item} className="rounded-full border border-white/10 bg-black/35 px-2.5 py-1.5">
                    {item}
                  </span>
                ))}
              </div>
          ...
          <div className="mt-6 flex flex-col gap-3">
            ...
            <details className="group overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm">
              <summary className="mono cursor-pointer select-none px-4 py-3 text-[0.65rem] uppercase tracking-[0.18em] text-[var(--muted)] transition-colors hover:text-[#ffd600]">
                Advanced: Scan Private Repositories (Optional)
              </summary>
              <div className="flex flex-col gap-3 border-t border-white/10 bg-black/25 p-4">
        ...
        {phase === "investigating" ? (
          ...
          >
            <div className="flex items-center justify-between gap-3 border-b border-white/10 pb-4">
              <div>
                <p className="mono text-xs uppercase tracking-[0.2em] text-[#ffd600]">
                  Live analysis
                </p>
              ...
              <Binary className="h-4 w-4 text-[#f7931a]" />
            </div>

            <div className="mt-4 grid gap-4 md:grid-cols-[1.6fr_1fr]">
              <div className="mono rounded-2xl border border-white/10 bg-black/45 p-4 text-xs leading-6 text-[#ffdca8]">
                {logs.map((line) => (
                  ...
                  {INVESTIGATION_LOGS.map((line, index) => {
                    ...
                    return (
                      <div key={line} className="flex items-center gap-2 text-xs">
                        <span
                          className={`h-2 w-2 rounded-full ${done ? "bg-[#f7931a] shadow-[0_0_12px_rgba(247,147,26,0.7)]" : "bg-zinc-700"}`}
                        />
